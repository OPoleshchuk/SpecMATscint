  ...
  fCheckOverlaps(true)
{
  ...
  sciHousSizeZ = sciCrystSizeZ + sciReflWindThick/2 + sciHousWindThick/2 + sciWindSizeZ;
  ...
  // Position of the gousing relative to the crystal position 
	...
	for (G4int iseg = 0; iseg < nbSegments ; iseg++) {
			...
			for (G4int icrystRow = 0; icrystRow < nbCrystInSegmentColumn; icrystRow++) {
				for (G4int icrystCol = 0; icrystCol < nbCrystInSegmentRow; icrystCol++) {
						...
						G4ThreeVector positionHous = (G4ThreeVector(0., 0., sciHousPosZ+1*mm) + positionInSegment);
			...
			G4ThreeVector positionSegment = (circleR1+sciHousSizeZ)*uz;
